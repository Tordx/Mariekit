<?php
namespace CrazyElements\Core\Settings\Base;
...
abstract class Manager {
	...
	public function __construct() {
		PrestaHelper::add_action( 'elementor/editor/init', [ $this, 'on_elementor_editor_init' ] );

		PrestaHelper::add_action( 'elementor/ajax/register_actions', [ $this, 'register_ajax_actions' ] );

		$name = $this->get_css_file_name();

		PrestaHelper::add_action( "elementor/css-file/{$name}/parse", [ $this, 'add_settings_css_rules' ] );
	}
	...
	final public function ajax_save_settings( $request ) {
		 ...
		 */
		$success_response_data = PrestaHelper::apply_filters( "elementor/settings/{$settings_name}/success_response_data", $success_response_data, $id, $data );
	...
	final public function save_settings( array $settings, $id = 0 ) {
		...
		$css_file = new Global_CSS( 'global.css' );
		if ( $css_file ) {
			$css_file->update();
		}
	}
	...
	protected function print_editor_template_content( $name ) {
		...
		<div class="elementor-panel-navigation">
			<# _.each( elementor.config.settings.<?php echo PrestaHelper::esc_html( $name ); ?>.tabs, function( tabTitle, tabSlug ) { #>
				<div class="elementor-panel-navigation-tab elementor-tab-control-{{ tabSlug }}" data-tab="{{ tabSlug }}">
					<a href="#">{{{ tabTitle }}}</a>
				</div>
				<# } ); #>
		</div>
		<div id="elementor-panel-<?php echo PrestaHelper::esc_attr( $name ); ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-elementor-panel-<?php echo PrestaHelper::esc_attr( $name ); ?>-settings">
