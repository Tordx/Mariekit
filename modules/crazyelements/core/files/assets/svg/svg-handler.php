<?php
namespace CrazyElements\Core\Files\Assets\Svg;
...
class Svg_Handler {
	...
	public static function is_svg_uploads_enabled() {
		return ! ! PrestaHelper::get_option( 'elementor_allow_svg', false );
	}
	...
	protected function get_meta() {
		return PrestaHelper::get_post_meta( $this->attachment_id, self::META_KEY, true );
	}
	...
	protected function update_meta( $meta ) {
		PrestaHelper::update_post_meta( $this->attachment_id, self::META_KEY, $meta );
	}
	...
	protected function delete_meta() {
		...
		PrestaHelper::delete_post_meta( $this->attachment_id, self::META_KEY );
	}
	...
	public static function get_inline_svg_curl($url){
		$ch = curl_init();
	...
	public static function get_inline_svg( $attachment_id ) {
		$svg = self::get_inline_svg_curl(self::get_escape_file_url($attachment_id));
		if (  empty( $svg ) ) {
			return "";
		}
		return $svg;
	...
	public static function get_escape_file_url($url){
	  $parts = parse_url($url);
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( 'svg' !== $ext ) {
			$file['error'] = sprintf( PrestaHelper::__( 'The uploaded %s file is not supported. Please upload a valid svg file', 'elementor' ), $ext );
			return $file;
		}

		if ( ! self::is_enabled() ) {
			$file['error'] = PrestaHelper::__( 'SVG file is not allowed for security reasons', 'elementor' );
			return $file;
		}

		if ( self::svg_sanitizer_can_run() && ! $this->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = PrestaHelper::__( 'Invalid SVG Format, file not uploaded for security reasons', 'elementor' );
		}
	...
	private function get_allowed_attributes() {
		...
		return PrestaHelper::apply_filters( 'elementor/files/svg/allowed_attributes', $allowed_attributes );
	}
	...
	private function get_allowed_elements() {
		...
		return PrestaHelper::apply_filters( 'elementor/files/svg/allowed_elements', $allowed_elements );
	}
	...
	public function wp_prepare_attachment_for_js( $attachment_data, $attachment, $meta ) {
		...
		$svg = self::get_inline_svg( $attachment->url );
		if ( ! $svg ) {
			return $attachment_data;
		}
	...
	public static function is_enabled() {
		...
		if ( null === $enabled ) {
			...
			$enabled = PrestaHelper::apply_filters( 'elementor/files/svg/enabled', $enabled );
		}
		return $enabled;
	}
	...
	public function __construct() {
		PrestaHelper::add_filter( 'upload_mimes', [ $this, 'upload_mimes' ] );
		PrestaHelper::add_filter( 'wp_handle_upload_prefilter', [ $this, 'wp_handle_upload_prefilter' ] );
		PrestaHelper::add_filter( 'wp_check_filetype_and_ext', [ $this, 'wp_check_filetype_and_ext' ], 10, 4 );
		PrestaHelper::add_filter( 'wp_prepare_attachment_for_js', [ $this, 'wp_prepare_attachment_for_js' ], 10, 3 );
		PrestaHelper::add_action( 'elementor/core/files/clear_cache', [ $this, 'delete_meta_cache' ] );
	}
}
